...
export function exportCustomersCSV(customers, transactions) {
  const headers = ['Name', 'Email', 'Address', 'DOB', 'Join Date', 'Points', 'Tier', 'Total Earned', 'Total Redeemed'];
  const rows = customers.map(c => {
    ...
    return [c.name, c.email, c.address || '', c.dob || '', c.joinDate, c.points, getTier(c.points).name, earned, redeemed];
  });
